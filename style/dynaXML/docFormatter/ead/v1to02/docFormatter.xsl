...
-->
	<xsl:param name='dtdpath'></xsl:param>
	<xsl:output 
  method="xml" 
  version="1.0" 
  ...
  doctype-public="+//ISBN 1-931666-00-8//DTD ead.dtd (Encoded Archival Description (EAD) Version 2002)//EN"
   doctype-system="http://www.oac.cdlib.org/ents/ead.dtd"
/>
...
-->
	<xsl:param name='mainagencycode'></xsl:param>
...
-->
	<xsl:param name='converter'>v1to02.cdl.xsl (2004-09-16)</xsl:param>
  ...
  match="*|comment()|processing-instruction()|text()">
	...
	<xsl:template match='eadheader'>
		<xsl:copy>
			<!-- eadid @ processing -->
			<xsl:for-each select='@*'>
<xsl:message>n:<xsl:value-of select="name()"/> t:<xsl:value-of select="text()"/>
. <xsl:value-of select="."/>
</xsl:message>
				<xsl:choose>
...
as both are not logically possible. -->
  ...
  -->
	...
	<xsl:template match='c[drow] |
		...
		<xsl:variable name='langlabel' select="'Language'"/>
		<!-- first copy c#  -->
		<xsl:copy>
			<xsl:apply-templates select='@*'/>
			<!-- then make a did -->
