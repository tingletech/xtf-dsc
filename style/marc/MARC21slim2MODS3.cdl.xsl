...
-->
	<xsl:template match="/">
		<xsl:apply-templates select="marc:record" mode="m2m"/>
	</xsl:template>

	<xsl:template name="marcRecord" match="marc:record" mode="m2m">
	 <mods version="3.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-0.xsd" xtf:meta="true" xtf:store="true" xtf:index="false">
		...
		<xsl:for-each select="marc:datafield[@tag=130]|marc:datafield[@tag=240]|marc:datafield[@tag=730][string(@ind2)!='2']">
		...
		<xsl:apply-templates select="marc:datafield[653 &gt;= @tag and @tag &gt;= 600]" mode="m2m"/>

		<xsl:apply-templates select="marc:datafield[@tag=656]" mode="m2m"/>
		...
		<xsl:for-each select="marc:datafield[@tag=730][string(@ind2)='2']">
			<relatedItem>
				<xsl:call-template name="constituentOrRelatedType"/>
				<titleInfo>
					<title>
		...
		<xsl:for-each select="marc:datafield[@tag=740][string(@ind2)='2']">
			<relatedItem>
				<xsl:call-template name="constituentOrRelatedType"/>
				<titleInfo>
					<title>
		...
		<!-- CDL: Added to contain Melvyl SYSID -->
		<xsl:for-each select="marc:controlfield[@tag=001]">
			<identifier type="SYSID">
				<xsl:value-of select="."/>
			</identifier>
		</xsl:for-each>
		...
		<recordInfo>
            ...
            <!-- CDL: Modified to use 901a and b rather than 001 & 003 -->
	...
	<xsl:template match="marc:datafield[@tag=600]" mode="m2m">
		<subject>
	...
	<xsl:template match="marc:datafield[@tag=610]" mode="m2m">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<name type="corporate">
				<xsl:for-each select="marc:subfield[@code='a']">
	...
	<xsl:template match="marc:datafield[@tag=611]" mode="m2m">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<name type="conference">
				<namePart>
	...
	<xsl:template match="marc:datafield[@tag=630]" mode="m2m">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<titleInfo>
				<title>
	...
	<xsl:template match="marc:datafield[@tag=650]" mode="m2m">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<topic>
				<xsl:call-template name="chopPunctuation">
	...
	<xsl:template match="marc:datafield[@tag=651]" mode="m2m">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<xsl:for-each select="marc:subfield[@code='a']">
	...
	<xsl:template match="marc:datafield[@tag=653]" mode="m2m">
		<subject>
			<xsl:for-each select="marc:subfield[@code='a']">
				<topic>
					<xsl:value-of select="."/>
	...
	<xsl:template match="marc:datafield[@tag=656]" mode="m2m">
		<subject>
			<xsl:if test="marc:subfield[@code='2']">
				<xsl:attribute name="authority">
					<xsl:value-of select="marc:subfield[@code='2']"/>
